...
def create_pbi_pr(pbi: dict, tasks: list, repo_path: Path) -> str:
    """
    Create a GitHub PR for a completed PBI branch using the gh CLI.
    Returns the PR URL on success, "" on failure.
    Requires: gh CLI installed and authenticated (gh auth login).
    """
    branch  = f"pbi/{pbi['id']}"
    title   = f"[PBI] {pbi['title']}"

    # Build a rich PR body from PBI spec + task summary
    task_lines = "\n".join(
        f"- [{t.get('status','?').upper()}] `{t['id']}` (score: {t.get('quality_score','?')}/10) — {t['description'][:80]}"
        for t in tasks
    )
    files_list = "\n".join(f"- `{f}`" for f in pbi.get("affected_files", []))
...
### Acceptance Criteria
{pbi.get('acceptance_criteria', '_None specified_')}

### Files Changed
{files_list or '_See diff_'}

### Tasks Completed
{task_lines}

---
_Auto-generated by Orchestrator. Review diff, then merge or request changes._
"""
    ...
    try:
        ...
        log.warning("gh CLI not found — install with: brew install gh && gh auth login")
        return ""
    except Exception as e:
        log.warning(f"PR creation failed: {e}")
        return ""
...
def _ensure_pbi_branch(repo_path: Path, pbi_id: str) -> bool:
    """
    Create and switch to branch pbi/{pbi_id} if it doesn't exist.
    If already on the branch, no-op. Returns True on success.
    PBI tasks commit here instead of main so the whole feature can be
    reviewed as one PR before merging.
    """
    branch = f"pbi/{pbi_id}"
    try:
        ...
        if cur.stdout.strip() == branch:
            return True
        # Create branch if missing, then switch
        ...
        log.warning(f"Could not switch to {branch}: {e} — committing to current branch")
        return False


def _return_to_main(repo_path: Path) -> None:
    """Switch back to main after committing a PBI task."""
...
def _auto_commit(task: dict, repo_path: Path) -> str:
    ...
    PBI tasks commit to branch pbi/{pbi_id} instead of main.
    Flat tasks (no pbi_id) commit directly to main.
    approval_required=True tasks are handled before this is called.
    ...
    # PBI tasks go to a feature branch
    on_pbi_branch = False
    if pbi_id:
        on_pbi_branch = _ensure_pbi_branch(repo_path, pbi_id)
        if on_pbi_branch:
    ...
    try:
        ...
        # Return to main after PBI branch commit so next task starts clean
        if on_pbi_branch:
            _return_to_main(repo_path)
